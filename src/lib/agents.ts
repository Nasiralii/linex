...
const PLATFORM_CONTEXT = `LineX-Forsa (لاينكس فرصة) is a trusted construction marketplace platform in Saudi Arabia.
...
// ============================================================================
export async function customerSupportAgent(message: string, locale: string = "ar", history: { role: string; content: string }[] = []) {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.5, max_tokens: 500,
    messages: [
      { role: "system", content: `You are the customer support agent for LineX-Forsa construction marketplace. Respond in ${lang}.
${PLATFORM_CONTEXT}
FAQ:
...
// ============================================================================
export async function projectIntakeAgent(message: string, locale: string = "ar", history: { role: string; content: string }[] = []) {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.7, max_tokens: 800,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a construction project intake consultant for LineX-Forsa. Respond in ${lang}.
...
If still gathering info: { "ready": false, "message": "your question to the user" }` },
  ...
  catch { return { ready: false, message: res.choices[0]?.message?.content || "" }; }
}

// ============================================================================
// Agent 3: Smart Bid Evaluator
// ============================================================================
export async function bidEvaluatorAgent(projectTitle: string, bids: any[], locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.3,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a construction bid evaluation expert. Analyze bids and provide recommendations in ${lang}.
Return JSON: { "recommendation": "top bid ID", "analysis": "detailed comparison", "risks": ["risk1"], "ranking": [{"bidId": "", "score": 0, "reason": ""}] }` },
      { role: "user", content: JSON.stringify({ project: projectTitle, bids }) },
    ],
  });
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { analysis: res.choices[0]?.message?.content }; }
}

// ============================================================================
// Agent 4: Admin Intelligence Agent
// ============================================================================
export async function adminIntelligenceAgent(stats: any, locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.5,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are an admin intelligence analyst for LineX-Forsa construction marketplace. Respond in ${lang}.
...
Return JSON: { "summary": "brief overview", "insights": ["insight1"], "alerts": ["alert1"], "recommendations": ["rec1"] }` },
  ...
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { summary: res.choices[0]?.message?.content }; }
}

// ============================================================================
// Agent 5: Outreach & Engagement Agent
// ============================================================================
export async function outreachAgent(project: any, contractor: any, locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.7, max_tokens: 300,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `Generate a personalized outreach message for a contractor/engineer about a new matching project on LineX-Forsa. 
...
// ============================================================================
export async function contractDraftingAgent(project: any, bid: any, contractType: string = "SIMPLE", locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.3, max_tokens: 2000,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a construction contract drafting specialist for Saudi Arabia. Respond in ${lang}.
...
Include: parties, scope of work, timeline, payment terms, quality standards, dispute resolution. Use Saudi Arabian legal standards.` },
  ...
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { terms: res.choices[0]?.message?.content }; }
}

// ============================================================================
// Agent 7: Bid Writing Assistant
// ============================================================================
export async function bidWritingAgent(projectDetails: any, contractorProfile: any, locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.7, max_tokens: 1000,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a bid writing consultant for construction contractors in Saudi Arabia. Respond in ${lang}.
...
Return JSON: { "proposalText": "", "proposalTextAr": "", "suggestedPrice": 0, "suggestedTimeline": 0, "tips": ["tip1"], "tipsAr": ["tip1 Arabic"] }` },
      { role: "user", content: JSON.stringify({ project: projectDetails, contractor: contractorProfile }) },
    ],
  });
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { proposalText: res.choices[0]?.message?.content }; }
}

// ============================================================================
// Agent 8: Document Verification Agent
// ============================================================================
export async function documentVerificationAgent(documentType: string, documentText: string, locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.2,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a document verification specialist for a Saudi Arabian construction marketplace. Respond in ${lang}.
...
Return JSON: { "isValid": true/false, "confidence": 0-100, "documentType": "", "findings": ["finding1"], "findingsAr": ["finding1 Arabic"], "recommendations": "", "recommendationsAr": "", "redFlags": ["flag1"] }` },
      { role: "user", content: JSON.stringify({ type: documentType, content: documentText }) },
    ],
  });
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { isValid: false, confidence: 0, findings: ["Unable to analyze"] }; }
}

// ============================================================================
// Agent 9: Price Estimation Agent
// ============================================================================
export async function priceEstimationAgent(projectDescription: string, locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.5,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a construction cost estimation expert for Saudi Arabia. Respond in ${lang}.
...
Be realistic with Saudi Arabia pricing. Consider material costs, labor, location premiums.` },
  ...
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { estimatedMin: 0, estimatedMax: 0, confidence: "low" }; }
}

// ============================================================================
// Agent 10: Review Sentiment Agent
// ============================================================================
export async function reviewSentimentAgent(reviews: any[], locale: string = "ar") {
  const lang = locale === "ar" ? "Arabic" : "English";
  const res = await ai().chat.completions.create({
    model: "gpt-4o-mini", temperature: 0.3,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a review analysis specialist. Respond in ${lang}.
...
Return JSON: { "overallSentiment": "positive|neutral|negative", "sentimentScore": 0-100, "summary": "", "summaryAr": "", "themes": ["theme1"], "themesAr": ["theme1 Arabic"], "suspiciousReviews": [{"reviewId": "", "reason": ""}], "reputationSummary": "", "reputationSummaryAr": "" }` },
  ...
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { overallSentiment: "neutral", sentimentScore: 50 }; }
}
...
export async function marketingAgent(request: {
  type: "instagram_post" | "instagram_story" | "twitter" | "linkedin" | "facebook" | "campaign";
  topic: string;
  locale: string;
}) {
  ...
  const res = await ai().chat.completions.create({
    ...
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: `You are a social media marketing expert for LineX-Forsa (لاينكس فرصة), a premium construction marketplace in Saudi Arabia. Respond in ${lang}.
${PLATFORM_CONTEXT}
...
For campaign: { "campaignName": "", "campaignNameAr": "", "objective": "", "targetAudience": "", "platforms": [], "contentCalendar": [{"day": "", "platform": "", "content": "", "contentAr": ""}], "kpis": [] }` },
  ...
  try { return JSON.parse(res.choices[0]?.message?.content || "{}"); }
  catch { return { error: "Failed to generate content" }; }
}
