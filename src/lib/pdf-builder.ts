...
export function buildContractPDF(data: ContractData): Buffer {
  ...
  doc.setFontSize(18);
  doc.text("Rasi - Contract", w / 2, 15, { align: "center" });
  doc.setFontSize(10);
  ...
  const details = [
    ...
    [
      "Award Amount:",
      `${data.awardedAmount.toLocaleString()} ${data.currency}`,
    ],
  ];
  doc.setFontSize(10);
  doc.setTextColor(26, 35, 50);
  ...
  drawSignatureBox(
    ...
    "Contractor",
    data.contractorName,
    data.contractorSignedAt,
  );

  // Footer
  doc.setFontSize(7);
  doc.setTextColor(170, 170, 170);
  doc.text("This contract is issued by Rasi Platform — rasi.sa", w / 2, 285, {
    align: "center",
  });
